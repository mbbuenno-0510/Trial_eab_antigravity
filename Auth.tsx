  ...
  const handleGoogleAuth = async () => {
    ...
    try {
      ...
      if (user) {
        ...
        if (!userDoc.exists) {
          // Se for um novo usuário via Google e estivermos na tela de registro, usa o perfil selecionado
          // Caso contrário, assume ADULT por padrão
          const profileToSave = view === 'REGISTER' && selectedProfile ? selectedProfile : ProfileType.ADULT;
          
          // Usuários Google não podem ser do tipo CHILD diretamente (exige login de responsável)
          if (profileToSave === ProfileType.CHILD) {
              await auth.signOut();
              throw new Error("Contas de criança devem ser criadas pelo formulário do responsável.");
          }

          const userData: any = {
            uid: user.uid,
            email: user.email,
            displayName: user.displayName || name || "Usuário Google",
            profileType: profileToSave,
            createdAt: Date.now(),
          };

          // Se for profissional/escola mas não preencheu os campos extras, podemos pedir depois 
          // ou preencher com placeholders se vier do Google
          if (profileToSave === ProfileType.PROFESSIONAL) {
              userData.professionalCouncil = professionalCouncil || 'CRM';
              userData.professionalCode = professionalCode || 'Pendente';
          }
          if (profileToSave === ProfileType.SCHOOL) {
              userData.cnpj = cnpj || 'Pendente';
          }
      ...
      >
        ...
        {(view === 'LOGIN' || view === 'REGISTER') && (
          <form onSubmit={handleAuth} className="space-y-4 animate-in fade-in slide-in-from-bottom-4 duration-300">
            ...
            {/* Google Login Button - MOSTRAR APENAS NO REGISTRO PARA DEFINIR PERFIL CORRETAMENTE OU SE JÁ FOR LOGIN */}
            {(view === 'REGISTER' && selectedProfile !== ProfileType.CHILD) && (
              <>
                <div className="relative my-6">
                    <div className="absolute inset-0 flex items-center"><div className="w-full border-t border-slate-200"></div></div>
                    <div className="relative flex justify-center text-xs uppercase"><span className="bg-white px-2 text-slate-400 font-bold">Ou use sua conta</span></div>
                </div>
                ...
                <button 
                    ...
                    disabled={loading}
                    className="w-full bg-white border-2 border-slate-100 py-3 rounded-xl shadow-md hover:shadow-lg transition-all flex items-center justify-center gap-3 group"
                >
                    ...
                    <span className="text-slate-700 font-bold text-sm">Cadastrar com Google</span>
                </button>
              </>
            )}
